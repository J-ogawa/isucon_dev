...
class App < Sinatra::Base
  ...
  get '/message' do
    ...
    statement = db.prepare('SELECT message.id,message.content,message.created_at,user.name,user.display_name,user.avatar_icon FROM message LEFT JOIN user ON message.user_id = user.id WHERE message.id > ? AND channel_id = ? ORDER BY message.id DESC LIMIT 100')
  ...
  get '/fetch' do
    ...
    channel_ids.each do |channel_id|
      ...
      r['unread'] = if row.nil?
        ...
        statement.execute(channel_id, row).first['cnt']
      end
      statement.close
      res << r
    end
  ...
  get '/history/:channel_id' do
    ...
    n = 20
    statement = db.prepare('SELECT message.id, message.content, message.created_at, user.name, user.display_name, user.avatar_icon FROM message LEFT JOIN user ON message.user_id = user.id WHERE channel_id = ? ORDER BY message.id DESC LIMIT ? OFFSET ?')
